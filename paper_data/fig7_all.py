...
if __name__ == "__main__":
    fig, axs = plt.subplots(4, 4, figsize=(28, 16), layout="constrained")
    handles = None
    ...
    for experiment in range(0, len(kafkas)):
        ...
        print(
            f"kafka p99 over 1s: {kafka_p99_over1}, epoch p99 over 1s: {sys_p99_over1}, ratio: {sys_p99_over1/kafka_p99_over1}"
        )
        ...
        for idx, t in enumerate(sys_p99):
            if (
                ...
                and sys_p50[idx] <= 400
                and t <= lat_max
                and abs(r2pc_p99_ratio[idx] - 1) <= allow
            ):
                target_idx = idx
        ...
        (l2,) = ax1.plot(
            kafka_in_tp,
            [int(row["p50"]) for row in kafka],
            label="Kafka Streams p50",
            marker=markers[1],
            color=colors[1],
            markersize=marksize,
        )
        (l4,) = ax2.plot(
            kafka_in_tp,
            [int(row["p99"]) for row in kafka],
            label="Kafka Streams p99",
            ls="--",
            marker=markers[1],
            color=colors[1],
            markersize=marksize,
        ...
        (l7,) = ax1.plot(
            r2pc_in_tp,
            r2pc_p50,
            label="Kafka Streams transaction on Impeller p50",
            marker=markers[3],
            color=colors[3],
            markersize=marksize,
        )
        (l8,) = ax2.plot(
            r2pc_in_tp,
            r2pc_p99,
            label="Kafka Streams transaction on Impeller p99",
            ls="--",
            marker=markers[3],
            color=colors[3],
            markersize=marksize,
        ...
        (l11,) = ax1.plot(
            ackpt_in_tp,
            ackpt_p50,
            label="Align chkpt on Impeller p50",
            marker=markers[4],
            color=colors[4],
            markersize=marksize,
        )
        (l12,) = ax2.plot(
            ackpt_in_tp,
            ackpt_p99,
            label="Align chkpt on Impeller p99",
            ls="--",
            marker=markers[4],
            color=colors[4],
            markersize=marksize,
    ...
    plt.savefig("fig7.pdf", bbox_inches="tight", pad_inches=0)
