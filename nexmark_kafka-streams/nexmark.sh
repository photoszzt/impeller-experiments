...
for ((j = 0; j < $NUM_SRC_INSTANCE; j++)); do
	...
	ssh -q $MANAGER_HOST -oStrictHostKeyChecking=no -- "docker service create \
        --mount type=bind,source=$WORKSPACE_DIR/impeller,destination=/src \
...
for HOST in $ALL_BROKER_HOSTS; do
	ssh -q $HOST -oStrictHostKeyChecking=no -- 'ls -l /dev/nvme*' >>$EXP_DIR/broker_sar/${HOST}_dev
...
for HOST in $ALL_SOURCE_HOSTS; do
	ssh -q $HOST -oStrictHostKeyChecking=no -- 'ls -l /dev/nvme*' >>$EXP_DIR/source_sar/${HOST}_dev
	ssh -q $HOST -oStrictHostKeyChecking=no -- sar -o /home/ubuntu/sar_st 1 >/dev/null 2>&1 &
done

mkdir -p $EXP_DIR/app_sar
for HOST in $ALL_APP_HOSTS; do
	ssh -q $HOST -oStrictHostKeyChecking=no -- 'ls -l /dev/nvme*' >>$EXP_DIR/app_sar/${HOST}_dev
	ssh -q $HOST -oStrictHostKeyChecking=no -- sar -o /home/ubuntu/sar_st 1 >/dev/null 2>&1 &
done
